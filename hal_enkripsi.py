...
# Fungsi untuk menyimpan teks terenkripsi ke database


def save_text(user_id, encrypted_text, key):
    conn = connect_db()
    cursor = conn.cursor()
    # Menggunakan kolom 'kunci' yang baru
    cursor.execute("INSERT INTO text (user, isi, kunci) VALUES (%s, %s, %s)",
                   (user_id, encrypted_text, key))
    conn.commit()
    conn.close()
...
def encryption_form():
    ...
    if st.button("Enkripsi"):
        if input_text and key:
            ...
            save_text(st.session_state.user_id, encrypted_text, key)
